...
class TableViewCellWithImage: UITableViewCell {
    ...
    override func layoutSubviews() {
        ...
        self.textLabel?.frame = CGRect(x: (self.imageView?.frame.size.width)! + 2 * decalage, y: 0.0, width: self.frame.size.width - (self.imageView?.frame.size.width)! - 3 * decalage - 30.0, height: self.frame.size.height)
